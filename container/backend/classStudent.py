...
def add_class_student(class_student):
    if class_student.equipment_id is None:
        ...
        result = run_sql_script(
            sql_script, (class_student.class_id, class_student.student_id)
        )
...
def get_classes_on_shift_of_student(student_id):
    sql_script = """
    SELECT * FROM class c JOIN shift s ON c.shift_id = s.id JOIN class_student cs ON c.id = cs.class_id WHERE cs.student_id = %s;
    """
    return run_sql_script(sql_script, (student_id,))["results"]
...
def modify_class_student_by_student_id(class_student_id, class_student):
    if class_student.equipment_id is None:
        ...
        result = run_sql_script(
            sql_script,
            (class_student.class_id, class_student.student_id, class_student_id),
        )
        return result["alerts"]
    else:
        sql_script = """
...
def modify_class_student_by_class_id(class_id, class_student):
    if class_student.equipment_id is None:
        ...
        """
        result = run_sql_script(
            sql_script, (class_student.class_id, class_student.student_id, class_id)
        )
        return result["alerts"]
    else:
        sql_script = """
